...
export class example extends plugin {
  constructor () {
    super({
      ...
      rule: [
                ...
                {
                    reg: `^(派蒙|凯亚|安柏|丽莎|琴|香菱|枫原万叶|迪卢克|温迪|可莉|早柚|托马|芭芭拉|优菈|云堇|钟离|魈|凝光|雷电将军|北斗|甘雨|七七|刻晴|神里绫华|雷泽|神里绫人|罗莎莉亚|阿贝多|八重神子|宵宫|荒泷一斗|九条裟罗|夜兰|珊瑚宫心海|五郎|达达利亚|莫娜|班尼特|申鹤|行秋|烟绯|久岐忍|辛焱|砂糖|胡桃|重云|菲谢尔|诺艾尔|迪奥娜|鹿野院平藏)社会语录`,
                    fnc: 'pmshyl'
                },
      ]
    })
  ...
  async pmshyl (e) {
    ...
    /** 一言接口地址 */
    let url = 'http://bg.suol.cc/API/shyl/'
    /** 调用接口获取数据 */
    let res = await fetch(url).catch((err) => logger.error(err))
    ...
    res = await res.text()
    /** 输入日志 */
    logger.info(`[接口结果] 派蒙社会语录：${res}`)
    let role = e.msg.replace("社会语录","");
    let url2 = `http://233366.proxy.nscc-gz.cn:8888/?text=${encodeURI(res)}&speaker=${encodeURI(role)}`;
    console.log(url2);
    await e.reply(segment.record(url2));
